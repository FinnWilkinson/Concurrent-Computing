...
#define SYS_STATUS    ( 0x08 )
#define SYS_CREATE    ( 0x09 )
#define SYS_SEND      ( 0x0A )
#define SYS_CHECKSTATUS     ( 0x0B )
#define SYS_FIND      ( 0x0C )
#define SYS_CHECKMESSAGE     ( 0x0D )
#define SYS_FINDCHAN  ( 0x0E )
...
// for process identified by pid, set status to x
extern void status( pid_t pid, int x );

//creates a new channel, returns the channel id
extern int create(int source, int destination);
//sends a message m down channel with id x, changes status to q
extern void send(int m, int x, int q);
//returns the channel status
extern int check_status(int x);
//finds the process's pcb pid
extern int find();
//returns the channel message
extern int check_message(int x);
//finds the process's channel, x is the process' id
extern int find_chan(int x);
